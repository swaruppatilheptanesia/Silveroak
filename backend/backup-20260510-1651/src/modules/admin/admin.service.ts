import { EligibilityRule, PostingType, Prisma, UserRole, VerificationStatus } from '@prisma/client';
...
export async function getUsers(tenantId: string, filters: QueryUsersInput) {
  ...
  const [users, total] = await Promise.all([
    prisma.user.findMany({
      ...
      orderBy: { [sort_by || 'name']: sort_order || 'asc' },
...
export async function getAuditLogs(tenantId: string, filters: QueryAuditLogsInput) {
  const { page, limit, module, action, user_id, sort_by, sort_order } = filters;
  const where: Record<string, unknown> = { tenant_id: tenantId };
...
export async function getStudents(tenantId: string, filters: QueryStudentsInput) {
  const { page, limit, department, batch, verification_status, search, min_cgpa, max_cgpa, sort_by, sort_order } = filters;
...
export async function getSelectionDatabase(tenantId: string, filters: QuerySelectionDatabaseInput) {
  ...
  const internshipRecords = internships.map((internship) => ({
    ...
    posting_type: internship.offer?.posting?.type ?? null,
...
export async function getInterestRegistrations(tenantId: string, filters: QueryInterestRegistrationsInput) {
  ...
  const conditions: Prisma.StudentWhereInput[] = [
    ...
    {
      interest_registrations: {
        some: interestTypeFilter ? { interest_type: interestTypeFilter } : {},
      },
    },
  ];
...
function buildStudentWhere(
  tenantId: string,
  filters: Pick<QueryStudentsInput, 'department' | 'batch' | 'verification_status' | 'search' | 'min_cgpa' | 'max_cgpa'>
...
function mapAdminStudent(student: AdminStudentSource) {
  ...
  return {
    ...
    skills: student.skills_profile
      ...
      : null,
    employment: student.current_employment
      ? {
          is_currently_working: student.current_employment.is_currently_working,
          employment_type: student.current_employment.employment_type,
          company_name: student.current_employment.company_name,
          designation: student.current_employment.designation,
          duration: student.current_employment.duration,
          package_lpa: decimalToNumber(student.current_employment.package_lpa),
          updated_at: student.current_employment.updated_at.toISOString(),
        }
      : null,
    ...
    noc_requests: student.noc_requests.map((noc) => ({
      ...
      end_date: noc.end_date.toISOString(),
